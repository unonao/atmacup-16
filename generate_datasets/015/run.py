...
def concat_session_candidate_feature(cfg, mode: str, candidate_df: pl.DataFrame):
    ...
    # transition probを追加
    yad2cand_df = pl.read_parquet(cfg.exp.node2vec_path)
    yad2yad_score = (
        yad2cand_df.explode(["neighbor_yad_nos", "cosine_similarity"])
        .with_columns(
            pl.col("yad_no").alias("from_yad_no"),
            pl.col("neighbor_yad_nos").alias("to_yad_no"),
            pl.col("cosine_similarity").alias("node2vec_cosine_similarity"),
        )
        .select(["from_yad_no", "to_yad_no", "node2vec_cosine_similarity"])
    )
    log_df = load_log_data(Path(cfg.dir.data_dir), mode)
    last_log_df = (
    ...
    last_log_prob_df = last_log_df.join(yad2yad_score, on="from_yad_no")
    candidate_df = candidate_df.join(
        last_log_prob_df,
