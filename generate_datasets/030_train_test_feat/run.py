...
def load_yad_data_with_features(cfg, mode):
    ...
    for path in cfg.exp.yad_feature_paths:
        feature_df = pl.read_parquet(Path(path) / f"yad_feature_{mode}.parquet")
        # counts とつくカラムは除外
        feature_df = feature_df.select(
            [col for col in feature_df.columns if "counts" not in col]
        )
...
def concat_session_feature(cfg, mode: str, candidate_df: pl.DataFrame):
    ...
    """
    log_df = load_log_data_first_all(cfg, mode)
    yad_df = load_yad_data_with_features(cfg, mode)
...
def concat_session_candidate_feature(cfg, mode: str, candidate_df: pl.DataFrame):
    ...
    for location_col in ["sml_cd", "lrg_cd", "ken_cd", "wid_cd"]:
        ...
        loc2loc_prob_df = pl.read_parquet(
            ...
            / f"{location_col}2{location_col}_feature_{mode}.parquet"
        )
    ...
    # transition probを追加
    yad2yad_prob = pl.read_parquet(
        Path(cfg.exp.transition_prob_path) / f"yad2yad_feature_{mode}.parquet"
    )
    log_df = load_log_data_first_all(cfg, mode)
    last_log_df = (
        log_df.group_by("session_id")
    ...
    prob_col = "transition_prob_split_transition_prob_fix/base"
    log_df = load_log_data_first_all(cfg, mode)
    log_df = (
    ...
    yad2yad_prob = pl.read_parquet(
        Path(cfg.exp.transition_prob_all_path) / f"yad2yad_feature_{mode}.parquet"
    )
    prob_col = "transition_prob_split_transition_prob_all_fix/base"
    log_df = load_log_data_first_all(cfg, mode)
    log_df = (
        log_df.sort(by="session_id").with_columns(pl.col("yad_no").alias("from_yad_no"))
    ).select(["session_id", "from_yad_no"])
    ...
    yad2yad_prob = pl.read_parquet(
        Path(cfg.exp.transition_prob_bidirect_all_path)
        / f"yad2yad_feature_{mode}.parquet"
    )
    prob_col = "transition_prob_split_transition_prob_bidirect_all_fix/base"
    log_df = load_log_data_first_all(cfg, mode)
    log_df = (
        log_df.sort(by="session_id").with_columns(pl.col("yad_no").alias("from_yad_no"))
    ).select(["session_id", "from_yad_no"])
