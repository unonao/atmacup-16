...
def load_and_union_candidates(cfg, mode: str):
    ...
    for candidate_info in cfg.exp.candidate_info_list:
        ...
        dfs.append(df)
    for session2item_candidate_info in cfg.exp.session2item_candidate_info_list:
        df = pl.read_parquet(
            Path(session2item_candidate_info["dir"]) / f"{mode}_candidate.parquet"
        ).select(["session_id", "candidates"])  # scoreカラムを落とす
        df = df.with_columns(
            pl.col("candidates")
            .cast(pl.List(pl.Int64))
            .list.head(session2item_candidate_info["max_num_candidates"])
            .alias("candidates")
        ).filter(pl.col("candidates").list.len() > 0)
        dfs.append(df)
...
def concat_session_feature(cfg, mode: str, candidate_df: pl.DataFrame):
    ...
    log_yad_df = log_yad_df.group_by(by="session_id").agg(
        ...
        + [pl.count("session_id").alias("session_count")]
    )
...
def concat_session2item_feature(cfg, mode: str, candidate_df: pl.DataFrame):
    for session2item_candidate_info in cfg.exp.session2item_candidate_info_list:
        df = pl.read_parquet(
            Path(session2item_candidate_info["dir"]) / f"{mode}_candidate.parquet"
        )
        df = df.explode(["candidates", f"{session2item_candidate_info.name}/scores"])
        # series_idごとの scores の rank も追加
        df = df.with_columns(
            pl.col(f"{session2item_candidate_info.name}/scores")
            .rank()
            .over("session_id")
            .alias(f"{session2item_candidate_info.name}/rank"),
            pl.col("candidates").cast(pl.Int64),
        )
        # 結合
        candidate_df = candidate_df.join(
            df, on=["session_id", "candidates"], how="left"
        )

    # factor情報を追加
    session_factor_df = pl.read_parquet(
        Path(cfg.exp.factor_dir) / "session_factor.parquet"
    )
    candidate_df = candidate_df.join(session_factor_df, on="session_id", how="left")

    yad_factor_df = pl.read_parquet(Path(cfg.exp.factor_dir) / "yad_factor.parquet")
    candidate_df = candidate_df.join(
        yad_factor_df, left_on="candidates", right_on="yad_no", how="left"
    )
...
def make_datasets(cfg, mode: str):
    ...
    candidate_df = concat_session2item_feature(cfg, mode, candidate_df)
    logger.info(f"concat_session2item_feature: {candidate_df.shape}")
