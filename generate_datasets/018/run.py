...
def concat_supervised_feature(cfg, mode: str, candidate_df: pl.DataFrame):
    # te transition probを追加
    te_transition_prob_df = pl.read_parquet(
        Path(cfg.exp.te_transition_prob_dir) / f"{mode}_yad2yad.parquet"
    )
    log_df = load_log_data(Path(cfg.dir.data_dir), mode)
    log_df = (
        log_df.group_by("session_id")
    ...
    if mode == "train":
        fold_df = pl.read_parquet(cfg.exp.fold_path)
        log_df = log_df.join(fold_df, on="session_id")
        last_log_prob_df = log_df.join(
            te_transition_prob_df, on=["from_yad_no", "fold"]
        ).drop("fold")
    elif mode == "test":
        last_log_prob_df = log_df.join(te_transition_prob_df, on="from_yad_no")
    candidate_df = (
        candidate_df.join(
            last_log_prob_df,
            left_on=["session_id", "candidates"],
        ...
        .drop("from_yad_no")
        .with_columns(pl.col("transition_prob").alias("te_transition_prob_base"))
    )

    # te transition probを追加(transition_prob_all_path)
    te_transition_prob_df = pl.read_parquet(
        Path(cfg.exp.te_transition_prob_all_dir) / f"{mode}_yad2yad.parquet"
    )
    ...
    if mode == "train":
        ...
        log_df = log_df.join(fold_df, on="session_id")
        log_prob_df = (
            log_df.join(te_transition_prob_df, on=["from_yad_no", "fold"])
            .group_by(["session_id", "to_yad_no"])
            .agg(pl.sum("transition_prob").alias("transition_prob"))
        ).drop("fold")
    elif mode == "test":
        log_prob_df = (
            log_df.join(te_transition_prob_df, on="from_yad_no")
            .group_by(["session_id", "to_yad_no"])
            .agg(pl.sum("transition_prob").alias("transition_prob"))
        )
    candidate_df = (
        ...
        .drop("from_yad_no")
        .with_columns(pl.col("transition_prob").alias("te_transition_prob_all"))
    )
...
def make_datasets(cfg, mode: str):
    ...
    candidate_df = concat_supervised_feature(cfg, mode, candidate_df)
    logger.info(f"concat_supervised_feature: {candidate_df.shape}")
