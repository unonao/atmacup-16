...
def concat_session2item_feature(cfg, mode: str, candidate_df: pl.DataFrame):
    ...
    for factor_dir in cfg.exp.session2yad_factor_dirs:
        # factor情報を追加
        session_factor_df = pl.read_parquet(Path(factor_dir) / "session_factor.parquet")
        candidate_df = candidate_df.join(session_factor_df, on="session_id", how="left")

        yad_factor_df = pl.read_parquet(Path(factor_dir) / "yad_factor.parquet")
        candidate_df = candidate_df.join(
