...
def concat_session2location_feature(cfg, mode: str, candidate_df: pl.DataFrame):
    for session2location_candidate_info in cfg.exp.session2location_candidate_info_list:
        df = pl.read_parquet(
            Path(session2location_candidate_info["dir"]) / f"{mode}_candidate.parquet"
        )
        df = df.explode(
            ["candidates", f"{session2location_candidate_info.name}/scores"]
        )
        # series_idごとの scores の rank も追加・candidates を location に変換
        df = df.with_columns(
            pl.col(f"{session2location_candidate_info.name}/scores")
            .rank()
            .over("session_id")
            .alias(f"{session2location_candidate_info.name}/rank"),
            pl.col("candidates").alias(session2location_candidate_info.location),
        ).drop("candidates")
        # 結合
        candidate_df = candidate_df.join(
            df, on=["session_id", session2location_candidate_info.location], how="left"
        )

    for factor_info in cfg.exp.session2location_factor_info_list:
        # factor情報を追加
        session_factor_df = pl.read_parquet(
            Path(factor_info.dir) / "session_factor.parquet"
        )
        candidate_df = candidate_df.join(session_factor_df, on="session_id", how="left")

        location_factor_df = pl.read_parquet(
            Path(factor_info.dir) / f"{factor_info.location}_factor.parquet"
        )
        candidate_df = candidate_df.join(
            location_factor_df, on=factor_info.location, how="left"
        )
...
def make_datasets(cfg, mode: str):
    ...
    candidate_df = concat_session2location_feature(cfg, mode, candidate_df)
    logger.info(f"concat_session2location_feature: {candidate_df.shape}")
