...
def main(cfg: DictConfig) -> None:
    ...
    wandb.init(
        project="atmaCup16-exp-first" if cfg.exp.only_first else "atmaCup16-exp-other",
        name=exp_name,
    ...
    for fold in range(num_folds):
        ...
        if cfg.exp.only_first:
            use_valid_fold_df = use_valid_fold_df.filter(pl.col("seq_no") == 0)
        else:
            use_valid_fold_df = use_valid_fold_df.filter(pl.col("seq_no") != 0)
        ...
        valid_pred_df = (
            ...
            .select(
                ["session_id", "candidates", "seq_no"],
            )
        )
        if cfg.exp.only_first:
            valid_pred_df = valid_pred_df.filter(pl.col("seq_no") == 0)
        else:
            valid_pred_df = valid_pred_df.filter(pl.col("seq_no") != 0)
    ...
    with utils.trace("make oof submission & scoring"):
        oof_pred_df = (
            ...
            .filter(pl.col("original") == 1)
            .select(
                ["session_id", "candidates", "pred", "seq_no"],
            )
        )
        oof_pred_df.write_parquet(output_path / "oof_pred.parquet")
        if cfg.exp.only_first:
            oof_pred_df = oof_pred_df.filter(pl.col("seq_no") == 0)
        else:
            oof_pred_df = oof_pred_df.filter(pl.col("seq_no") != 0)
    ...
    with utils.trace("make test submission"):
        test_pred_df = (
            ...
            .sort(by=["session_id", "pred"], descending=True).with_columns()
            .select(
                ["session_id", "candidates", "pred", "seq_no"],
            )
        )
