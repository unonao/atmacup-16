...
def main(cfg: DictConfig) -> None:
    ...
    for fold in range(num_folds):
        ...
        use_train_fold_df = train_fold_df
        use_valid_fold_df = valid_fold_df.filter(pl.col("original") == 1)
        if cfg.exp.only_first:
            use_train_fold_df = use_train_fold_df.filter(pl.col("session_count") == 1)
            use_valid_fold_df = use_valid_fold_df.filter(pl.col("session_count") == 1)
        else:
            use_train_fold_df = use_train_fold_df.filter(pl.col("session_count") != 1)
            use_valid_fold_df = use_valid_fold_df.filter(pl.col("session_count") != 1)
        bst = train_one_fold(cfg, use_train_fold_df, use_valid_fold_df)
        ...
        if cfg.exp.only_first:
            valid_pred_df = valid_pred_df.filter(pl.col("session_count") == 1)
        else:
            valid_pred_df = valid_pred_df.filter(pl.col("session_count") != 1)
    ...
    with utils.trace("make oof submission & scoring"):
        ...
        if cfg.exp.only_first:
            oof_pred_df = oof_pred_df.filter(pl.col("session_count") == 1)
        else:
            oof_pred_df = oof_pred_df.filter(pl.col("session_count") != 1)
