...
def train_one_fold(
    ...
    if cfg.exp.lgbm.params.objective == "lambdarank":
        train_group = train_df["session_id"].value_counts(parallel=True).sort(by='session_id')['counts'].to_list()
        valid_group = valid_df["session_id"].value_counts(parallel=True).sort(by='session_id')['counts'].to_list()
...
def main(cfg: DictConfig) -> None:
    ...
    for fold in range(num_folds):
        ...
        valid_pred_df = (
            valid_fold_df.select(
                ["session_id", "candidates"],
            )
            .with_columns(pl.Series(name="pred", values=y_valid_pred))
